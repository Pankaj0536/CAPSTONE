...
def run_experiment():
    ...
    for count in counts:
        # Generate random queue for this run (allowing replacement if count > diskSize)
        if count <= disk_size:
        ...
        counts_res = {}
        for algo in algos:
            if algo == "FCFS": counts_res[algo] = fcfs(queue, initial_head, disk_size)
            elif algo == "SSTF": counts_res[algo] = sstf(queue, initial_head, disk_size)
            elif algo == "SCAN": counts_res[algo] = scan(queue, initial_head, disk_size)
            elif algo == "C-SCAN": counts_res[algo] = c_scan(queue, initial_head, disk_size)
            elif algo == "LOOK": counts_res[algo] = look(queue, initial_head, disk_size)
            elif algo == "C-LOOK": counts_res[algo] = c_look(queue, initial_head, disk_size)
            elif algo == "N-STEP SCAN": counts_res[algo] = n_step_scan(queue, initial_head, disk_size)
            elif algo == "F-SCAN": counts_res[algo] = f_scan(queue, initial_head, disk_size)
